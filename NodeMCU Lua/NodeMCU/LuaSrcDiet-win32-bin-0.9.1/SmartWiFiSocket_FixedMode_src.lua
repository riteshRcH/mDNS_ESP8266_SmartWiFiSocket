...
local mdns_hostname = fd:read()
fd:close()

fd = file.open('version', 'r')
local socketSoftwareVersion = fd:read()
fd:close()
...
tmrSyncStateWithCloudForInternetMode:register(660000, tmr.ALARM_AUTO, function(timer)
                        ...
                        http.get(get_url, nil, function(code, data)
                                if code == 200 and data then
                                    ...
                                    if not iv:find('[^0-9dfhjlnprtvxzEGIKNOQSUWY]') then
                                        local desired_state = crypto.decrypt("AES-CBC", "PwSeRVeRdAta#92*", encoder.fromHex(cipher_data), iv):match("(.-)%z*$"):sub(1, cipher_data_len):match("^%s*(.-)%s*$"):match('^([0-9])$')
                                        
                                        if desired_state then
                                           stop_running_timer_and_set_desired_state(desired_state == '1')
                                        end
                                    end
...
local identify_and_schedule_todays_schedule_timer_entries = function()
    ...
    gpio_write_setup_led_pins(not cron_scheduled_after_sntp, cron_scheduled_after_sntp, false)
                ...
                conn:on("receive", function(client_conn, payload)
                        ...
                        if method == 'GET' then
                            ...
                            elseif path == '/gpio/relay/stop_running_timer' then
                                ...
                                client_conn:send(stop_running_timer_and_set_desired_state(state_after_stopping_running_timer == '1'))
...
local internet_disconnected_handler = function()
    cron.reset()
    tmrSyncStateWithCloudForInternetMode:stop()
