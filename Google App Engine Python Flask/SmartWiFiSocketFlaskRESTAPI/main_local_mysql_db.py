...
maria_db_name = 'unify_smart_wifi_socket'
...
def add_update_user_for_app():
    try:
        conn = MySQLdb.connect(host=maria_db_host, database=maria_db_name, user=maria_db_app_user, password=maria_db_app_password, autocommit = False)
        cursor = conn.cursor()
...
def sockets_per_user_status_for_app():
    try:
        ...
        if google_oauth_id_token and validate_google_id_token(google_oauth_id_token, status_requestor_email):
            socket_status_read_connection_for_app = MySQLdb.connect(host=maria_db_host, database=maria_db_name, user=maria_db_app_user, password=maria_db_app_password, autocommit = True)
...
def update_sockets_per_user_for_socket():
    try:
        conn = MySQLdb.connect(host=maria_db_host, database=maria_db_name, user=maria_db_socket_user, password=maria_db_socket_password, autocommit = False)
        cursor = conn.cursor()
        ...
        input_values[6] = datetime.utcnow().strftime("%s")
...
def sockets_per_user_status_for_socket():
    try:
        ...
        socket_status_read_connection_for_socket = MySQLdb.connect(host=maria_db_host, database=maria_db_name, user=maria_db_socket_user, password=maria_db_socket_password, autocommit = True)
